   ...
   for( y = 0 ; y < HIGHT ; y++ )
   {
      ...
      for( x = 0 ; x < WIDTH ; x++)
      {
         ...
         for(s = 0; s < sphereNum; s++)
         {
            ...
            else
            {
               ...
               if(posT > 0 && posT < minT)
               {
                  ...
                  onSphere = 1;
                  onFloor  = 0;
                  rgb[y][x][0] = red[s]/2.0;   //half color is ambient
                  rgb[y][x][1] = green[s]/2.0;
                  rgb[y][x][2] = blue[s]/2.0;
         ...
         if(gradient)
         {
            ...
            if(dot > 0)
            {
               rgb[y][x][0] += dot*rgb[y][x][0];
               rgb[y][x][1] += dot*rgb[y][x][1];
               rgb[y][x][2] += dot*rgb[y][x][2];
            }
         }
      }
   }
