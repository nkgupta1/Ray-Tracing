...
int main(void)
{
   ...
   //
   for( frame; frame < frameNum ; frame++)
   {
      cxA[0] = frame * 1. / frameNum;
      for( y = 0 ; y < HIGHT ; y++ )
      {
         // printf("%d\n", y);
      ...
      sprintf( filename , "frame%03d.png" , frame ) ;
      //
      sprintf( ccommand , "convert output.ppm %s" , filename ) ;
      //
      system( ccommand ) ;
   ...
   for( frame; frame >= 0 ; frame--)
   {
      ...
      sprintf( filename , "frame%03d.png" , frameNum * 2 - frame ) ;
      //
      sprintf( ccommand , "convert output.ppm %s" , filename ) ;
      //
      system( ccommand ) ;
   ...
   system( "convert -delay 10 -loop -1 *png movie.gif" ) ;
   system( "rm *.png") ;
